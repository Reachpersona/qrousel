...
The presets deliberately span both QR treatments. They were once all pale enough to tint, which made every one of them a near-white indistinguishable from having no background - the palest was 1.08:1 against a white page, so tapping it appeared to do nothing. `background.test.js` now holds them to a perceptual distance in OKLab rather than a contrast ratio, because contrast measures lightness alone and would call a green and a blue of equal lightness identical. The swatches carry no visible text, so `backgroundLabel` names the current choice under the row - a preset's name, the hex for a picked colour, or `none`.
