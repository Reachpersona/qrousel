...
`prefers-color-scheme` sets the default page colours in `ContactCarousel.css`; an entry that names a background overrides both schemes inline, because the colour is the owner's choice about their card, not a theme for the viewer's phone. `index.css` declares `color-scheme: light dark` - without it Chrome on Android auto-darkens the page (and can filter images), which would recolour a chosen background, and it turns itself on when battery saver flips the system to dark.
